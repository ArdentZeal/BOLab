...
public class UserDAO extends AbstractDAO {
	...
	private void updateUserInTransaction(User user) {
		...
		// koennte einen Aenderungsstatus besitzen.
			...
			for (Iterator<User> iter = user.getFriendsOf().iterator(); iter.hasNext(); ) {
				...
				// Man kann nicht seineigener Freund sein.
	 ...
	 */
	public List<User> findFriendsOfUsers(User user) {
		String select = "SELECT * FROM member m, member_member mm WHERE m.id = mm.friendsOf_id and mm.member_id = " + user.getId();
	 ...
	 * Achtung: Bei zyklischen Abhaengigenkeiten ("in bin mein eigener Freund")
	 * fuert das zum Stack-Uebwerlauf!
	 ...
	 */
	private void fillFriendsOf(User user) {
		String select = "SELECT * FROM member m, member_member mm WHERE m.id = mm.member_id and mm.friendsOf_id = " + user.getId();
		
		MapSqlParameterSource params = new MapSqlParameterSource();

		List<User> friendsOf =  getNamedParameterJdbcTemplate().query(select, params, new UserRowMapper());
		user.getFriendsOf().clear();
